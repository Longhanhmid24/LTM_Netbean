...
public class NetworkService {
    ...
    private static String SERVER_IP = "10.15.4.231";
    ...
    static {
        ...
        m1.setPropertyNamingStrategy(PropertyNamingStrategies.SNAKE_CASE);
        SimpleModule ldm1 = new SimpleModule();
        ldm1.addDeserializer(ZonedDateTime.class, new LenientZonedDateTimeDeserializer());
        m1.registerModule(ldm1);
        m1.configure(com.fasterxml.jackson.databind.DeserializationFeature.FAIL_ON_UNKNOWN_PROPERTIES, false);
        MAPPER = m1;
        ObjectMapper m2 = new ObjectMapper();
        m2.registerModule(new JavaTimeModule());
        m2.disable(SerializationFeature.WRITE_DATES_AS_TIMESTAMPS);
        SimpleModule ldm2 = new SimpleModule();
        ldm2.addDeserializer(ZonedDateTime.class, new LenientZonedDateTimeDeserializer());
        m2.registerModule(ldm2);
        m2.configure(com.fasterxml.jackson.databind.DeserializationFeature.FAIL_ON_UNKNOWN_PROPERTIES, false);
        STOMP_MAPPER = m2;
    ...
    public static String createE2EESendableJson(MessageSendDTO dto) {
        try {
            return SEND_MAPPER.writeValueAsString(dto);
        } catch (Exception e) {
            return "{}";
        }
    }

    public static String createGroupSendableJson(GroupMessage gm) {
        try {
            return SEND_MAPPER.writeValueAsString(gm);
        } catch (Exception e) {
            return "{}";
        }
    }

    public static ChatMessage parseStompMessage(String json) {
        if (json == null || json.isEmpty()) {
            return null;
        }
        try {
            return STOMP_MAPPER.readValue(json, ChatMessage.class);
        } catch (Exception e) {
            System.err.println("Lỗi parseSTOMP: " + e.getMessage() + " | JSON: " + json);
            return null;
        }
    }

    public static GroupMessage parseGroupStompMessage(String json) {
        if (json == null || json.isEmpty()) {
            return null;
        }
        try {
            return STOMP_MAPPER.readValue(json, GroupMessage.class);
        } catch (Exception e) {
            return null;
        }
    }
        ...
        return CompletableFuture.supplyAsync(() -> {
            try {
                ...
                if (response.statusCode() == 200) {
                    ...
                    try {
                        JsonNode errorNode = STOMP_MAPPER.readTree(response.body());
        ...
        return CompletableFuture.supplyAsync(() -> {
            try {
                ...
                if (rp.statusCode() == 200) {
                    return MAPPER.readValue(rp.body(), new TypeReference<List<User>>() {
                    });
                }
            } catch (Exception e) {
            }
                ...
                if (rp.statusCode() == 200) {
                    ...
                    for (Map<String, Object> row : dbResult) {
                        ...
                        if (cipherObj instanceof String) {
                            msg.setContent((String) cipherObj);
                        } else if (cipherObj instanceof byte[]) {
                            msg.setContent(Base64.getEncoder().encodeToString((byte[]) cipherObj));
                        }
                        ...
                        if (tsObj instanceof String) {
                            try {
                                msg.setTimestamp(LocalDateTime.parse((String) tsObj, DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss.S")).atZone(ZoneId.systemDefault()));
                            } catch (Exception e) {
                                try {
                                    msg.setTimestamp(LocalDateTime.parse((String) tsObj, DateTimeFormatter.ISO_LOCAL_DATE_TIME).atZone(ZoneId.systemDefault()));
                                } catch (Exception e2) {
                                    try {
                                        msg.setTimestamp(ZonedDateTime.parse((String) tsObj));
                                    } catch (Exception e3) {
                                    }
                                }
                            }
    ...
    public static CompletableFuture<String> uploadFile(File file) {
        return CompletableFuture.supplyAsync(() -> {
            try {
                ...
                byte[] sp = sb.toString().getBytes(StandardCharsets.UTF_8);
                byte[] eb = ("\r\n--" + b + "--\r\n").getBytes(StandardCharsets.UTF_8);
                byte[] body = new byte[sp.length + fb.length + eb.length];
                System.arraycopy(sp, 0, body, 0, sp.length);
                System.arraycopy(fb, 0, body, sp.length, fb.length);
                System.arraycopy(eb, 0, body, sp.length + fb.length, eb.length);
                HttpRequest r = HttpRequest.newBuilder().uri(URI.create(BASE_URL + "upload")).header("Content-Type", "multipart/form-data; boundary=" + b).timeout(java.time.Duration.ofMinutes(2)).POST(HttpRequest.BodyPublishers.ofByteArray(body)).build();
                HttpResponse<String> rp = httpClient.send(r, HttpResponse.BodyHandlers.ofString());
                if (rp.statusCode() == 200) {
                    JsonNode root = MAPPER.readTree(rp.body());
                    if (root.has("url")) {
        ...
        return CompletableFuture.supplyAsync(() -> {
            try {
                ...
                if (rp.statusCode() == 200) {
                    ...
                    return dbResult.stream().map(row -> {
                        User friend = new User();
                        friend.setId((Integer) row.get("friend_id"));
                        friend.setUsername((String) row.get("username"));
                        friend.setAvatar((String) row.get("avatar"));
                        return friend;
    ...
    public static CompletableFuture<List<FriendRequest>> getFriendRequests(int userId) {
        return CompletableFuture.supplyAsync(() -> {
            try {
                ...
                HttpResponse<String> rp = httpClient.send(r, HttpResponse.BodyHandlers.ofString());
                if (rp.statusCode() == 200) {
                    return MAPPER.readValue(rp.body(), new TypeReference<List<FriendRequest>>() {
                    });
                }
            } catch (Exception e) {
                e.printStackTrace();
    ...
    public static CompletableFuture<Boolean> sendFriendRequest(int senderId, int receiverId) {
        return CompletableFuture.supplyAsync(() -> {
            try {
                ...
                HttpRequest r = HttpRequest.newBuilder().uri(URI.create(BASE_URL + "friendships/request")).header("Content-Type", "application/json").timeout(java.time.Duration.ofSeconds(10)).POST(HttpRequest.BodyPublishers.ofString(jsonBody)).build();
                HttpResponse<String> rp = httpClient.send(r, HttpResponse.BodyHandlers.ofString());
    ...
    public static CompletableFuture<Boolean> acceptFriendRequest(int senderId, int receiverId, int actionUserId) {
        return CompletableFuture.supplyAsync(() -> {
            try {
                ...
                String jsonBody = req.toJson();
                HttpRequest r = HttpRequest.newBuilder().uri(URI.create(BASE_URL + "friendships/accept")).header("Content-Type", "application/json").timeout(java.time.Duration.ofSeconds(10)).PUT(HttpRequest.BodyPublishers.ofString(jsonBody)).build();
                HttpResponse<String> rp = httpClient.send(r, HttpResponse.BodyHandlers.ofString());
                return rp.statusCode() == 200;
            } catch (Exception e) {
                e.printStackTrace();
    ...
    public static CompletableFuture<Boolean> removeFriend(int userId1, int userId2) {
        return CompletableFuture.supplyAsync(() -> {
            try {
                ...
                String jsonBody = req.toJson();
                HttpRequest r = HttpRequest.newBuilder().uri(URI.create(BASE_URL + "friendships/remove")).header("Content-Type", "application/json").timeout(java.time.Duration.ofSeconds(10)).method("DELETE", HttpRequest.BodyPublishers.ofString(jsonBody)).build();
                HttpResponse<String> rp = httpClient.send(r, HttpResponse.BodyHandlers.ofString());
                return rp.statusCode() == 200;
            } catch (Exception e) {
                e.printStackTrace();
    ...
    public static CompletableFuture<Long> startCall(int callerId, int receiverId, String callType) {
        ...
        return httpPostJson("/api/calls/start", body)
                .thenApply(json -> {
                    try {
                        return json.get("id").asLong(); // Spring return id
                    } catch (Exception e) {
                        System.err.println("⚠️ startCall: could not parse id, defaulting 0");
                        return 0L;
                    }
                });
    }

    // ✅ End call (PUT /api/calls/end/{id})
    public static CompletableFuture<Void> endCall(long callId) {
        return httpPut("/api/calls/end/" + callId, Map.of())
                .thenApply(res -> null);
    }

    public static CompletableFuture<com.fasterxml.jackson.databind.JsonNode> httpPostJson(String path, Map<String, Object> body) {
        try {
            ...
            return client.sendAsync(request, HttpResponse.BodyHandlers.ofString())
                    .thenApply(resp -> {
                        try {
                            ...
                            throw new RuntimeException("JSON parse error: " + resp.body());
                        }
                    });
        } catch (Exception e) {
            CompletableFuture<com.fasterxml.jackson.databind.JsonNode> f = new CompletableFuture<>();
            f.completeExceptionally(e);
            return f;
        }
    }
    
    public static CallSignal parseCallSignal(String json) {
        try {
            ObjectMapper mapper = new ObjectMapper();
            return mapper.readValue(json, CallSignal.class);
        } catch (Exception e) {
            e.printStackTrace();
            return null;
        }
    ...
    public static CompletableFuture<Void> httpPut(String path, Map<?, ?> body) {
        try {
            String url = API_BASE_URL + path;
    ...
    public static CompletableFuture<List<Group>> getGroupsForUser(int userId) {
        ...
        return CompletableFuture.supplyAsync(() -> {
            try {
                HttpRequest r = HttpRequest.newBuilder().uri(URI.create(ep)).timeout(java.time.Duration.ofSeconds(15)).GET().build();
                HttpResponse<String> rp = httpClient.send(r, HttpResponse.BodyHandlers.ofString());
                if (rp.statusCode() == 200) {
                    return MAPPER.readValue(rp.body(), new TypeReference<List<Group>>() {
                    });
                }
            } catch (Exception e) {
                e.printStackTrace();
    ...
    public static CompletableFuture<List<GroupMessage>> getGroupMessages(int groupId) {
        ...
        return CompletableFuture.supplyAsync(() -> {
            try {
                HttpRequest r = HttpRequest.newBuilder().uri(URI.create(ep)).timeout(java.time.Duration.ofSeconds(20)).GET().build();
                HttpResponse<String> rp = httpClient.send(r, HttpResponse.BodyHandlers.ofString());
                if (rp.statusCode() == 200) {
                    return MAPPER.readValue(rp.body(), new TypeReference<List<GroupMessage>>() {
                    });
                }
            } catch (Exception e) {
                e.printStackTrace();
    ...
    public static CompletableFuture<List<GroupMember>> getGroupMembers(int groupId) {
        return CompletableFuture.supplyAsync(() -> {
            try {
                ...
                HttpResponse<String> rp = httpClient.send(r, HttpResponse.BodyHandlers.ofString());
                if (rp.statusCode() == 200) {
                    return MAPPER.readValue(rp.body(), new TypeReference<List<GroupMember>>() {
                    });
                }
            } catch (Exception e) {
                e.printStackTrace();
    ...
    public static WebSocketClient connectWS(int userId, WSMessageListener listener) {
        ...
        WebSocketClient wc = new WebSocketClient(u) {
            @Override
            public void onOpen(ServerHandshake h) {
                send("CONNECT\naccept-version:1.1,1.0\nheart-beat:10000,10000\n\n\0");
                listener.setWebSocketClient(this);
            }
            ...
            public void onMessage(String m) {
                ...
                } else if (m.startsWith("MESSAGE\n")) {
                    int hEnd = m.indexOf("\n\n");
                    if (hEnd != -1) {
                        String h = m.substring(0, hEnd);
                        int bStart = hEnd + 2;
                        int bEnd = m.indexOf('\0', bStart);
                        if (bEnd > bStart) {
                            String b = m.substring(bStart, bEnd);
                            listener.onMessageReceived(h, b);
                        }
                    }
                } else if (m.startsWith("ERROR\n")) {
                    System.err.println("STOMP ERROR:" + m);
                } else if (!m.trim().isEmpty() && !m.equals("\n")) {/* Ignore keep-alive */
                }
            }
            ...
            public void onClose(int c, String r, boolean re) {
                ...
                if (c != 1000) {
                    ...
                    t.schedule(new TimerTask() {
                        public void run() {
                            NetworkService.connectWS(userId, listener);
                        }
                    }, 5000);
                }
            }
    ...
    public interface WSMessageListener {
        ...
        void setWebSocketClient(WebSocketClient c);
    }
    ...
    public static CompletableFuture<PublicKey> getPublicKey(int fId) {
        ...
        return CompletableFuture.supplyAsync(() -> {
            try {
                HttpRequest r = HttpRequest.newBuilder().uri(URI.create(BASE_URL + "keys/" + fId)).timeout(java.time.Duration.ofSeconds(10)).GET().build();
                HttpResponse<String> rp = httpClient.send(r, HttpResponse.BodyHandlers.ofString());
                if (rp.statusCode() == 200) {
                    JsonNode root = MAPPER.readTree(rp.body());
                    if (root.has("public_key")) {
                        String k = root.get("public_key").asText();
                        if (k != null && !k.isEmpty()) {
                            PublicKey pk = CryptoService.stringToPublicKey(k);
                            if (pk != null) {
                                KeyService.cachePublicRSAKey(fId, pk);
                                return pk;
                            }
                        }
                    }
    ...
    public static CompletableFuture<Boolean> uploadPublicKey(int uId, String pkStr) {
        return CompletableFuture.supplyAsync(() -> {
            try {
                ...
                HttpResponse<String> rp = httpClient.send(r, HttpResponse.BodyHandlers.ofString());
                return rp.statusCode() == 200 || rp.statusCode() == 201;
            } catch (Exception e) {
                return false;
            }
        });
